...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
		...
		topLabel.textAlignment=NSTextAlignmentCenter;//UITextAlignmentCenter;
		...
		bottomLabel.textAlignment=NSTextAlignmentCenter;//UITextAlignmentCenter;
		image=nil;
...
-(void) showInView:(UIView *)view withAnimation:(HUDAnimation)animation {
	...
	switch (animation) {
		case HUDAnimationNone:
            ...
            if (database.moveSpiral) {
                self.center=CGPointMake(view.bounds.size.width/2.0, view.bounds.size.height/2.0 + 150);
                database.moveSpiral = nil;
            }
            else {
